            ...
            <div className="block md:flex md:flex-row items-start justify-center md:space-x-16 absolute top-56 md:top-1/2 left-1/2 -translate-x-1/2 md:-translate-y-1/2 w-4/5">
                {productCards.map((card, index) => (
                    <div className="w-full md:w-1/3 text-center mb-16 md:mb-0" key={index}>
                        ...
                        <div className="bg-linkBlack px-2 py-3 rounded-3xl mt-10 md:mt-20">
                            <Link to={'products/' + card.path}>
                                <h2 className="uppercase text-primaryGreen hover:text-primaryBlue transition-all font-poppins font-semibold text-lg md:text-2xl lg:text-4xl mb-2">
                                    {card.name}
                                </h2>
                            </Link>
                            <span className="uppercase font-poppins font-medium text-lg text-gray-300">{card.subName}</span>
                        </div>
                    </div>
                ))}
            </div>
